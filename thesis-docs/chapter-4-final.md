...
**Validation Tools.** The Spiral Model's risk-analysis cycles used existing telemetry from the Hinaing backend (`backend/backend/data/metrics/`), published benchmark studies for vector-store latency and embedding accuracy, and architectural stress testing against the 19-agent target topology. Live infrastructure prototyping provided the decisive validation signal: early deployments on Railway's Free Tier triggered Out-Of-Memory errors under parallel agent execution, requiring a migration to HuggingFace Spaces (16 GB RAM), which successfully sustained the concurrent `asyncio.gather` and `ThreadPoolExecutor` patterns required by the 7-node topology. The 7-node count was validated against the requirement to automate every stage of the civic intelligence pipeline end-to-end — autonomous query planning, multi-platform ingestion, epistemic memory recall, multi-signal sentiment and credibility analysis, memory consolidation, domain-specific insight generation, and executive reporting — where each stage demands a distinct functional unit that cannot be collapsed without sacrificing latency, credibility verification, or thematic depth. The 19-agent count was validated by decomposing each node's functional requirements: seven core execution stages require specialist coordination (7), six civic themes require independent domain experts (6), five orthogonal credibility signals require independent verifiers (5), and post-generation NLI faithfulness checking requires a dedicated stage (1). No fewer agents can cover all four requirement classes without creating a single bottleneck agent that would serialize the pipeline and break the sub-5-minute latency target. Final selections were validated by whether they could be integrated into this 7-node LangGraph DAG while maintaining sub-5-minute end-to-end latency.
...
**Agentic Context Engineering (Stanford ACE).** Node 1's `QueryOrchestratorAgent` is inspired by Stanford's Agentic Context Engineering (ACE) framework (Zhang et al., 2025), which demonstrated that LLMs can improve their own context through reflection. Where ACE uses a 3-role Generator–Reflector–Curator loop to evolve context from scratch, Hinaing injects pre-defined Baguio civic domain ontology and temporal calendar awareness into the ReAct loop. The agent still reflects on its outputs, but the reflection is guided by domain tools (`get_domain_context`, `get_temporal_context`, `validate_query_diversity`) rather than learned from blank slate.

**Everything is Context (Context as Infrastructure).** A parallel line of existing research reframes context not as a transient prompt artifact but as governed, persistent infrastructure. Xu et al. (2025) propose that "everything is context" — all context artifacts (memory, tools, knowledge, human input) should be managed through a unified file system abstraction offering uniform mounting, metadata, and access control. Hinaing diverges from this unified abstraction in scope but matches it in intent through a distributed governance architecture. The 5-signal source-quality filter (domain trust, internal semantic cross-reference via BGE embeddings, Google Fact Check, LLM analysis, and Tavily web verification) governs external context before it enters the pipeline, while VSEE (Vector-Symbolic Epistemic Entailment) governs internal context by verifying semantic coherence across retrieved documents. This dual governance layer — external credibility scoring plus internal entailment verification — ensures that only context artifacts passing both thresholds propagate to the Theme and Coordinator agents, achieving traceability and accountability without a single unified mounting point. Hinaing's approach prioritizes domain-specific governance (civic credibility signals) over generic file-system access control, because the relevant quality dimensions for civic intelligence (source trust, factual consistency, thematic relevance) are domain-specific rather than generically enforceable through permissions.

**Qdrant over GraphRAG.** The study selected Qdrant Cloud rather than Microsoft GraphRAG for persistent vector storage because GraphRAG's construction phase creates significant indexing overhead that conflicts with the Self-Learning Cyclic RAG requirement of immediate write-back after analysis. Qdrant's storage layer supports real-time upsert and cosine similarity search and consistently ranks among the lowest-latency purpose-built vector databases in independent 2025–2026 benchmarks (e.g., 2–4 ms p50 query latency), while delivering sub-5-minute end-to-end latency in our own deployment. In addition, GraphRAG is designed for local deployment, which consumes local disk and compute resources over time; Qdrant Cloud provides managed persistence with automatic backups, aligning better with the operational requirements of a civic monitoring service.
...
**Logic-Infused Knowledge Graph QA** The study also compared against Logic-Infused Knowledge Graph QA (Bashir et al., 2025, *Data & Knowledge Engineering*, Volume 157, https://doi.org/10.1016/j.datak.2025.102406), which uses Prolog ontologies for domain reasoning. While Prolog-GraphRAG provides strong logical inference, it requires complete re-indexing for temporal updates and depends on external ontological schemas that are brittle for hyper-local civic issues. Hinaing replaces strict ontological schemas with Vector-Symbolic Epistemic Entailment (VSEE) and Temporal-Aware Reciprocal Rank Fusion (TA-RRF), which compute truth through vector-space consensus and dynamic temporal decay without re-indexing.

### 4.1.3 Agentic Harness Engineering

The framework contributes to *agentic harness engineering*—the emerging discipline of designing software infrastructure that wraps foundation models to make them reliable, actionable, and safe to use (OpenAI, 2026; Hashimoto, 2026; Osmani, 2026; Fowler, 2026). Unlike model optimization or prompt engineering, agentic harness engineering focuses on the architectural "scaffolding" that constrains model behavior, enforces guardrails, and enables autonomous multi-step workflows. The hinaing framework exemplifies this paradigm: its architectural scaffolding is deliberately *configurable at the structural level*, not merely tunable. Developers can substitute any LLM or embedding model through the provider factory, reweight or extend the credibility and sentiment signal ensembles, and—because the 7-node pipeline is assembled from discrete `add_node` functions with a factory/registry pattern for agents—add new execution nodes or register new domain/verification agents without rewriting the coordination layer. The Self-Learning Cyclic RAG loop is a backend-agnostic pattern (currently instantiated on Qdrant Cloud with BGE-large-en-v1.5 hybrid search), so the retrieval backend—including alternatives such as GraphRAG or vectorless approaches like PageIndex—can be substituted by providing a compatible store implementation that supports upsert and recall of enriched documents. This makes the harness extensible to new civic domains by code-level composition rather than by hyperparameter tuning. The concrete topology, agent decomposition, and cyclic-RAG loop are detailed in Section 4.2 as part of the framework's identified components.

### 4.1.4 Ensemble Method Scope

Within this harness, the framework employs two distinct ensemble architectures, both presented as validated design instantiations within the broader architectural contribution, not as the primary contribution itself. The sentiment analysis layer uses a dual-model neural ensemble (RoBERTa 40%, Gemini 2.5 Flash 60%) to balance social-media-native precision with civic-context understanding. The credibility verification layer uses a 5-signal ensemble — domain trust scoring, internal semantic cross-reference via BGE embeddings, Google Fact Check Tools API, LLM-based content analysis, and Tavily-based real-time web verification — that combines external semantic web search verification with internal VSEE (Vector-Symbolic Epistemic Entailment) verification to produce a unified agentic credibility score. Ensemble methods consistently outperform single models through three established mechanisms: statistical variance reduction across unstable base learners (Dietterich, 2000; Breiman, 1996), representational expansion beyond any single hypothesis (Dietterich, 2000), and diversity-driven error correction where uncorrelated classifier errors cancel out (Kuncheva & Whitaker, 2003). Opitz & Maclin (1999) empirically demonstrate this across 23 datasets: the ensemble nearly always beats the best single classifier. For sentiment analysis specifically, heterogeneous ensembles combining complementary architectures achieve superior accuracy because the two models fail on different examples (Minaee et al., 2019), and ensemble gains are largest on noisy social media text where individual models are most unstable (Alsayat, 2022). At the agent level, multi-agent systems function as ensembles through division of labor, redundancy, and diversity of perspectives (Tran et al., 2025), and performance scales with agent *diversity* rather than count: heterogeneous (diverse) agents contribute complementary, non-redundant evidence, whereas redundant homogeneous agents saturate early (Yang et al., 2026).

Hyperparameter tuning and model selection for either ensemble are explicitly out of scope for this thesis; the research does not attempt to discover the optimal LLM or weight configuration, only to demonstrate that the harness architecture supports pluggable, configurable ensembles as a structural feature. The framework is architected as a **blueprint**: developers can substitute any sentiment model, embedding model, LLM provider, or credibility signal weighting through configuration without altering the underlying architecture. The thesis defends the **structural novelty**—the 7-node topology, Self-Learning Cyclic RAG, and the dual-ensemble harness design (sentiment + credibility)—as the primary contribution for autonomous public opinion analysis and civic governance.
...
A component is defined as a modular, specialized functional unit that works in coordination with others within a goal-driven, autonomous architecture to act and learn. Unlike traditional AI, these components are agentic because they act autonomously, often using Large Language Models to break down high-level goals into smaller steps, maintain context, and interact with external systems. The system identifies a federated architecture of 19 specialized agents organized into a 7-Node Agentic Graph. The framework is architected as a **blueprint framework**: developers can substitute any LLM or embedding model through the provider factory, reconfigure the credibility and sentiment ensembles, and extend or reconfigure the node-level topology to target new civic domains without rewriting the coordination layer; the retrieval backend is swappable via a compatible store implementation (see the backend-agnostic cyclic-RAG note above). Because the pipeline is released as a reusable LangGraph architecture rather than a closed product, other projects can adopt the same 7-node harness and configure only the components they need—models, signals, themes, or target domains—for their own autonomous analysis workloads. The core Self-Learning Cyclic RAG method—the read-write memory loop that consolidates enriched documents and reuses their sentiment, credibility, and metadata across cycles—is a backend-agnostic pattern: it is currently instantiated on Qdrant Cloud with BGE-large-en-v1.5 hybrid search, but the same cyclic learning and Smart Reuse logic can be layered over alternative retrieval backends such as GraphRAG or vectorless approaches like PageIndex, since it depends only on a store that supports upsert and recall of enriched documents.
...
The six domain theme agents—Infrastructure, Health, Safety, Tourism, Economy, and Environment—were selected to align with the six mandated sectoral chapters that Philippine LGUs must include in their Comprehensive Development Plan under the Local Government Code of 1991 (RA 7160, Book III, Section 16) and the DILG CDP Preparation Guidelines (MC No. 2021-144). This legal framework is operationally reinforced by active DILG memorandum circulars: disaster risk reduction and management for health (MC 2025-074) and safer cities (MC 2026-029) for safety and health; and the DRRM/CCA capacity development strategy (MC 2026-014) for integrated resilience. The Philippine Development Plan 2023–2028 (NEDA, 2022) structures its six development outcome chapters—Infrastructure, Health, Public Safety and DRRM, Tourism, Economic Growth and Livelihoods, and Environment and Climate Action—around these same domains, with Baguio City's CDP 2023–2027 and CLUP 2014–2023 aligned to this national framework. Empirical studies confirm each domain as a distinct civic concern cluster in Baguio: the city's own institutional livability assessment system, validated against WHO's four dimensions, confirms infrastructure/convenience, health, safety, and environmental quality as measurable civic outcomes (Abacahan et al., 2026).

**Pipeline Coverage.** The seven-node topology was designed to fully automate the end-to-end civic intelligence pipeline without human intervention: Node 1 (autonomous query planning via `get_domain_context`) → Node 2 (multi-platform data ingestion) → Node 3 (epistemic memory recall from Qdrant) → Node 4 (concurrent sentiment and credibility analysis) → Node 5 (memory consolidation back to Qdrant) → Node 6 (domain-specific insight generation via Theme Sub-Agents) → Node 7 (executive reporting with NLI-verified faithfulness). No single node can be collapsed into another without breaking the pipeline: merging ingestion and analysis would prevent memory recall from enriching the analysis; merging analysis and consolidation would prevent Smart Reuse from caching enriched documents; merging theme specialization and reporting would eliminate the domain-specific actionable insights that distinguish Hinaing from generic summarization. The 19-agent count reflects this granularity: each execution stage requires specialist coordination (7), each civic domain requires independent reasoning (6), each credibility dimension requires independent verification (5), and post-generation faithfulness checking requires a dedicated NLI stage (1).
...
For real-time public opinion analysis, the framework uses a dual-model neural ensemble for sentiment. This model fuses a statistical transformer—RoBERTa, specifically the `twitter-roberta-base-sentiment-latest` model, with the neural large language model Gemini 2.5 Flash. Neuro-symbolic ensemble models combine deep neural networks with symbolic knowledge bases to enhance sentiment analysis performance. This is proven by empirical results from ensemble models using LSTM networks which achieved superior accuracy on Twitter, Amazon and Yelp datasets. (Alsaya et al., 2021) The rationale for this dual-model approach is to balance RoBERTa's strength in processing social-media-native slang (the `twitter-roberta-base-sentiment-latest` model is fine-tuned on the TweetEval benchmark and trained on ~124M tweets) with Gemini's deep contextual understanding of Baguio civic issues. The resulting ensemble uses a weighted distribution presented in the table below to provide more context-aware sentiment quantification than single-model alternatives.

The framework as a whole is neuro-symbolic at the system level: neural models (RoBERTa, Gemini, DeBERTa-v3, BGE embeddings) execute the perception and reasoning stages, while symbolic governance structures — the 7-node DAG topology, VSEE (Vector-Symbolic Epistemic Entailment) verification, TA-RRF temporal fusion, the 5-signal credibility rules, and the civic domain ontology — constrain and verify their outputs. The sentiment ensemble itself is purely neural; the symbolic layer operates at the architecture level rather than inside the sentiment pair.
...
**Context Engineering via Domain Ontology.** The `get_domain_context` tool (Node 1) retrieves two signal types: (1) the theme-specific keyword set from the civic domain ontology (10–18 Baguio-specific concern phrases per theme, e.g., "Baguio traffic congestion," "BGH dengue outbreak"), and (2) previously cached discoveries from Qdrant's Self-Learning Cyclic RAG for the same theme. The QueryOrchestratorAgent merges both signals into the ReAct loop, so generated queries simultaneously reflect what Baguio citizens are currently discussing and what the system has already learned from prior cycles. This dual grounding — real-time keyword injection plus historical memory recall — operationalizes the "everything is context" principle (Xu et al., 2025) within the constraints of a vector-store-backed multi-agent architecture, where context governance is achieved through schema contracts (the `focus_area`, `sentiment`, and `credibility_score` metadata attributes) rather than file system permissions.
...
**Objective 1** confirmed that existing multi-agent and RAG components—LangGraph DAG runtime, BGE-large-en-v1.5 embeddings, Qdrant Cloud persistent storage, and a dual-model neural ensemble sentiment analysis—provide the architectural substrate for real-time civic intelligence. Each component was selected through the Spiral Model's comparative analysis and validated against latency, customization, and cloud-persistence criteria. Key differentiators include LangGraph's node-level flexibility over Google ADK, Qdrant's cloud-native performance over GraphRAG's local indexing overhead, and the Smart Reuse optimization over RAGCache's retrieval-only caching.

**Objective 2** produced a 19-agent hierarchical federated multi-agent system organized into a 7-node Self-Learning Cyclic RAG pipeline. The 19 agents span seven execution stages, six civic themes, five orthogonal credibility signals, and one post-generation verification stage. The architecture executes concurrent I/O-bound operations via `asyncio.gather` and parallel CPU-bound operations via `ThreadPoolExecutor`, achieving predictable sub-5-minute latency through analysis consolidation. Critically, the pipeline automates the entire civic-intelligence workflow end-to-end with minimal human intervention—from autonomous query planning and multi-platform ingestion through epistemic memory recall, multi-signal analysis, memory consolidation, domain-specific insight generation, and executive reporting—realizing the autonomy objective stated in the research problem.
